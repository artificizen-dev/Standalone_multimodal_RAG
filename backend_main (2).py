...
def upload_and_process_pdf(file_path: str, file_name: str):
    ...
    try:
        ...
        # Track uploaded PDFs
        uploaded_pdfs[file_name] = file_path
        return True
    except Exception as e:
...
# Video-related functions (existing code)
...
async def query_video(request: QueryVideoRequest):
    ...
    try:
        ...
        if relevant_segment:
            ...
            raise HTTPException(status_code=404, detail="No relevant segment found")
    except Exception as e:
...
async def upload_pdf(file: UploadFile = File(...)):
    try:
        ...
        # Alternatively, URL-encode the file name
...
@app.post("/query-pdf/")
async def query_pdf(request: QueryPDFRequest):
    file_name = request.file_name
    query = request.query

    try:
        if file_name not in uploaded_pdfs:
            raise HTTPException(status_code=404, detail="PDF not found")

        response = query_documents(query, file_name)
